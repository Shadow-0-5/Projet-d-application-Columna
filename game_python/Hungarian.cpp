...
double HungarianAlgorithm::SolveStatique(double costMatrix[144]) {
    double cost = 0.0;
    int assignment[4];
    double transformedCostMatrix[144];

    for (int i = 0; i < 144; ++i) {
        transformedCostMatrix[i] = -costMatrix[i];
    }

    assignmentoptimal(assignment, &cost, transformedCostMatrix, 4, 36);
